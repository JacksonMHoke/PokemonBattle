...
class Burned(Status):
    ...
    def trigger(self, context, eventContext, trigger):
        if trigger not in self.triggers or self.owner.state!=State.ACTIVE:
            return
        self.owner.takeDamage(dmg=self.dmg, context=context)
...
class Frozen(Status):
    ...
    def trigger(self, context, eventContext, trigger):
        if trigger not in self.triggers or context.attacker!=self.owner:
            return
        if random()<self.unfreezeChance:
            self.owner.status=None
            context.window['combatLog'].update(f'{self.owner.name} was unfrozen!\n', append=True)
        else:
            eventContext.cancelMove=True
            context.window['combatLog'].update(f'{self.owner.name} is frozen and cannot move!\n', append=True)
...
class Paralyzed(Status):
    ...
    def trigger(self, context, eventContext, trigger):
        if trigger==Trigger.BEFORE_MOVE and context.attacker==self.owner:
            if random()<self.paraChance:
                eventContext.cancelMove=True
                context.window['combatLog'].update(f'{self.owner.name} was stuck in paralysis!\n', append=True)
        if trigger==Trigger.AFTER_STATUS and context.defender==self.owner:
            self.owner.stats.addMult('SPE', -self.speedDebuff)
            context.window['combatLog'].update(f'{self.owner.name} has been slowed by paralysis!\n', append=True)
    ...
    def trigger(self, context, eventContext, trigger):
        if trigger==Trigger.BEFORE_MOVE and context.attacker==self.owner:
            if random()<self.wakeupChance or self.turnsAsleep>=self.maxTurns:
                context.window['combatLog'].update(f'{self.owner.name} woke up!\n', append=True)
                self.owner.status=None
                return
            eventContext.cancelMove=True
            context.window['combatLog'].update(f'{self.owner.name} is fast asleep!\n', append=True)
            self.turnsAsleep+=1